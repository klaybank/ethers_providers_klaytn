...
export class BaseProvider extends Provider implements EnsProvider {
  ...
  async waitForTransaction(
    ...
    console.log(
      "[BASE-PROVIDER] waitForTransaction transactionHash : ",
      transactionHash
    );
    console.log(
      "[BASE-PROVIDER] waitForTransaction confirmations : ",
      confirmations
    );
  ...
  _wrapTransaction(tx: Transaction, hash?: string): TransactionResponse {
    ...
    result.wait = async (confirmations?: number) => {
      console.log(
        "[BASE-PROVIDER] _wrapTransaction confirmations : ",
        confirmations
      );
    ...
    if (isHexString(blockHashOrBlockTag, 32)) {
      ...
      try {
        params.blockTag = this.formatter.blockTag(
          await this._getBlockTag(blockHashOrBlockTag)
        );
  ...
  async getTransactionReceipt(
    ...
    console.log(
      "[BASE-PROVIDER] getTransactionReceipt transactionHash : ",
      transactionHash
    );
    await this.getNetwork();
    ...
    return poll(
      async () => {
        ...
        console.log(
          "[BASE-PROVIDER] getTransactionReceipt receipt : ",
          receipt
        );
